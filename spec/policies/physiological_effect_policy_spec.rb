...
RSpec.describe PhysiologicalEffectPolicy do
  let(:admin) { create(:user, :admin) }
  let(:member) { create(:user) }
  let(:investigator) { create(:user, :investigator) }
  let(:sighting) { create(:sighting, submitter: member) }
  let(:effect) { create(:physiological_effect, sighting: sighting) }
  ...
  permissions :create? do
    it "grants create access to admins" do
      expect(described_class).to permit(admin, PhysiologicalEffect.new)
    end

    it "grants create access to members" do
      expect(described_class).to permit(member, PhysiologicalEffect.new)
    end

    it "grants create access to investigators" do
      expect(described_class).to permit(investigator, PhysiologicalEffect.new)
    end
  end
  ...
  describe described_class::Scope do
    let!(:effects) { create_list(:physiological_effect, 3) }

    it "returns all effects for admins" do
      resolved_scope = described_class.new(admin, PhysiologicalEffect.all).resolve
      expect(resolved_scope).to match_array(effects)
    end

    it "returns all effects for members" do
      resolved_scope = described_class.new(member, PhysiologicalEffect.all).resolve
      expect(resolved_scope).to match_array(effects)
    end
  end
end
