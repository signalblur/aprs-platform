...
RSpec.describe EquipmentEffectPolicy do
  ...
  permissions :create? do
    it "grants create access to admins" do
      expect(described_class).to permit(admin, EquipmentEffect.new)
    end

    it "grants create access to members" do
      expect(described_class).to permit(member, EquipmentEffect.new)
    end

    it "grants create access to investigators" do
      expect(described_class).to permit(investigator, EquipmentEffect.new)
    end
  end
  ...
  describe described_class::Scope do
    let!(:effects) { create_list(:equipment_effect, 3) }

    it "returns all effects for admins" do
      resolved_scope = described_class.new(admin, EquipmentEffect.all).resolve
      expect(resolved_scope).to match_array(effects)
    end

    it "returns all effects for members" do
      resolved_scope = described_class.new(member, EquipmentEffect.all).resolve
      expect(resolved_scope).to match_array(effects)
    end
  end
end
